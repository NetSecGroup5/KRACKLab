...
def openShellForPing():
    if sta1 is not None:
        result=sta1.cmd("ping -c 20 fakeAp")
        print(result)
    global exeBtn
    if exeBtn is not None:
        exeBtn.config(fg="green")
...
stepDictionary: dict[int,stepInfo] = {
    1: stepInfo("Introduction (step 1/8)",1,"First, let's discuss about our network topology. In this exercise there will be 2 wireless station: \n\n - sta1, which simulated the victim; \n - fakeAp, a fake access point which will run a modified version of a script created by Mathy Vanhoef \n (who discovered the KRACK vulnerability) for detecting vulnerable devices. \n\n The script will run a modified version of hostapd, a program normally used to use a \n normal network card as an Access Point, that will automatically reject the 4th message \n of the handshake. \n\n The idea is that the script will create an Access Point to which sta1 will connect, then the same script will forward the repeated message 3 of \n the handshake that is continuously generated by hostapd (since, remember, has been modified \n just for never receiving the 4th message of the handshake) to sta1 and, finally, it will analyze the \n 4th message of the handshake before this will be \"rejected\" by hostapd to see if sta1 reinstalled the key \n (and, consequentially, resetted the packet number) and/or installed an all-zero key.",None,None,None,False,None,None),
